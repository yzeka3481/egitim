...
const FALLBACK_VIDEOS = [
    'https://files.vidstack.io/sprite-fight/720p.mp4', // Reliable test video
    // Original: 'https://static.videezy.com/system/resources/previews/000/004/954/original/ink_drops_02.mp4',
];
...
async function fetchHistoryData(dateStr) {
    // Check for local file protocol - Fetch API won't work due to CORS
    if (window.location.protocol === 'file:') {
        console.warn('Running locally (file://), using mock data to avoid CORS errors.');
    ...
    // Utilizes history.muffinlabs.com API which is free and open
    ...
    try {
        ...
        // Remove common stopwords and pick longest words
        const words = randomEvent.text.split(' ')
            .filter(w => w.length > 5)
            .map(w => w.replace(/[.,;()]/g, ''));

        const keyword = words.length > 0 ? words[0] : 'history';

        return {
            text: `TARİHTE BUGÜN (${day}.${month}.${randomEvent.year})\n\n${randomEvent.text}`,
            keywords: keyword,
        ...
        return {
            text: `TARİHTE BUGÜN (${day}.${month})\n\n1969 yılında İnsanlık Ay'a ilk kez ayak bastı.`,
            keywords: 'space',
            year: '1969'
        };
    }
